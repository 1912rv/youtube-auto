...
Complete working local & GitHub Actions pipeline for Lichess puzzle Shorts:
...
6. Automatically updates the YOUTUBE_TOKEN_JSON secret on GitHub repository secrets if refreshed.
"""
...
# ---------------- CONFIGURATION ----------------
OUTPUT_DIR = Path(
    os.environ.get("OUTPUT_DIR", Path(tempfile.gettempdir()) / "youtube-auto")
)
...
def convert_san_to_speech(side_text, san_move, announce_time_up=True):
  if not san_move or san_move == "N/A":
    return (
        f"{side_text}. Can you spot the best move here?",
        "Time's up. I couldn't find a solution.",
    )
...
def san_move_to_spoken_text(san_move):
  ...
  return (
      f"pawn takes {san_clean.replace('x', '')}"
      if "x" in san_clean
      else f"pawn to {san_clean}"
  )


def generate_chess_move_sound(
    filename="chess_move.wav", sample_rate=44100
):
  duration = 0.12
  t = np.linspace(0, duration, int(sample_rate * duration), False)
  envelope = np.exp(-35 * t)
  tone = (
      0.7 * np.sin(2 * np.pi * 220 * t) + 0.3 * np.sin(2 * np.pi * 440 * t)
  ) * envelope
  ...
  wavfile.write(
      filename, sample_rate, np.column_stack((audio_int16, audio_int16))
  )
  return filename


def generate_background_sound(
    filename="background.wav", sample_rate=44100, duration=20.0
):
  ...
  soft_kick = np.sin(2 * np.pi * 82 * time_axis) * np.exp(
      -18 * ((time_axis * beat) % 1)
  )
  envelope = np.minimum(1.0, time_axis / 1.5) * np.minimum(
      1.0, (duration - time_axis) / 1.5
  )
  pulse = 0.85 + 0.15 * np.sin(2 * np.pi * beat * time_axis)
  mix = (
      0.032 * pad
      + 0.018 * bass
      + 0.009 * melody * pluck_decay
      + 0.006 * soft_kick
  ) * pulse
...
def fetch_puzzle_from_lichess():
  ...
  if not candidates:
    raise RuntimeError(
        f"Could not fetch a new Lichess puzzle after {PUZZLE_FETCH_ATTEMPTS}"
        " attempts. No video was generated to avoid reusing an old puzzle."
    )
...
def get_board_from_puzzle_json(puzzle_json):
  ...
  if game and 0 <= initial_ply < len(moves):
    ...
    last_move = (
        chess.Move.from_uci(last_move_uci)
        if last_move_uci
        else moves[initial_ply]
    )
...
def get_piece_image(symbol, size):
  ...
  try:
    ...
    png_data = cairosvg.svg2png(
        bytestring=res.content, output_width=size, output_height=size
    )
...
def generate_board_pil(board, arrows=None, size_px=BOARD_SIZE, perspective=None):
  ...
  for file_index in range(8):
    for rank_index in range(8):
      ...
      draw.rectangle(
          ...
          fill=(
              light_square if (file_index + rank_index) % 2 == 0 else dark_square
          ),
      )
  ...
  for arrow in arrows or []:
    ...
    if perspective == chess.WHITE:
      start = (
          (start_file + 0.5) * square_size,
          (7 - start_rank + 0.5) * square_size,
      )
      end = ((end_file + 0.5) * square_size, (7 - end_rank + 0.5) * square_size)
    else:
      start = (
          (7 - start_file + 0.5) * square_size,
          (start_rank + 0.5) * square_size,
      )
...
def create_reel_frame_array(
  ...
  board_resized = board_pil.resize(
      (BOARD_SIZE, BOARD_SIZE), Image.Resampling.LANCZOS
  )
  ...
  if last_move_text:
    draw.rounded_rectangle(
        (70, 290, 1010, 365), radius=18, fill="#3A2615", outline="#FFB86C", width=3
    )
    draw_centered_text(
        draw, last_move_text, 306, get_font(34), WIDTH, fill="#FFD18A"
    )
  if info_text:
  ...
  draw_centered_text(
      ...
      get_font(54),
      WIDTH,
      fill="#00FF7F" if is_solution else "#FFFFFF",
  )

  if puzzle_link:
    draw_centered_text(
        draw,
        f"Puzzle: {puzzle_link}",
        1570,
        get_font(30),
        WIDTH,
        fill="#00BFFF",
...
# ---------------- GITHUB SECRET AUTOMATIC UPDATE ----------------
def update_github_secret(new_token_json_str):
  """Updates YOUTUBE_TOKEN_JSON on GitHub repository via API so tokens never expire."""
  github_repo = os.environ.get("GITHUB_REPOSITORY")
  github_token = os.environ.get("REPOSITORY_SERVICE_TOKEN") or os.environ.get(
      "GITHUB_TOKEN"
  )

  if not github_repo or not github_token:
    print(
        "ℹ️ GitHub repository info/token not found in environment. Skipping"
        " secret auto-update."
    )
    return
  ...
  try:
    ...
    print(
        "⚠️ PyNaCl package not found. Skipping GitHub Secret update. Install via"
        " 'pip install pynacl'."
    )
    return
  ...
  # 1. Get Public Key from Repository
  key_url = (
  ...
  if res.status_code != 200:
    print(
        f"⚠️ Failed to fetch GitHub public key: {res.status_code} - {res.text}"
    )
    return
  ...
  encrypted = sealed_box.encrypt(new_token_json_str.encode("utf-8"))
  encrypted_b64 = base64.b64encode(encrypted).decode("utf-8")

  # 3. Put Secret to GitHub
  secret_url = f"https://api.github.com/repos/{github_repo}/actions/secrets/YOUTUBE_TOKEN_JSON"
  payload = {"encrypted_value": encrypted_b64, "key_id": key_id}
  put_res = requests.put(secret_url, headers=headers, json=payload)

  if put_res.status_code in (201, 204):
    print("✅ Successfully updated YOUTUBE_TOKEN_JSON on GitHub Secrets!")
  else:
    print(
        f"⚠️ Failed to update GitHub Secret: {put_res.status_code} -"
        f" {put_res.text}"
    )
...
def refresh_youtube_credentials(creds):
  if (
      not creds
      or not getattr(creds, "expired", False)
      or not getattr(creds, "refresh_token", None)
  ):
    return creds
  ...
  try:
    ...
    raise RuntimeError(
        "YouTube OAuth token has expired or been revoked. Regenerate"
        " YOUTUBE_TOKEN_JSON from a Google account with YouTube upload"
        " permission."
    ) from exc


def save_refreshed_credentials(creds):
  """Exports active token data to JSON and updates GitHub Secrets if active in GitHub Actions."""
  token_data = {
      "token": creds.token,
  ...
  print("💾 Saved active credentials to updated_token.json.")

  # Push updated secret directly to GitHub repository
  update_github_secret(token_json_str)
    ...
    if creds and creds.expired and creds.refresh_token:
      ...
      if not client_secret_json:
        raise FileNotFoundError(
            ...
            " YOUTUBE_CLIENT_SECRET_JSON for local OAuth login."
        )
      flow = InstalledAppFlow.from_client_config(
          json.loads(client_secret_json), SCOPES
      )
...
def run_pipeline():
  ...
  board, solution_moves, last_opponent_move = get_board_from_puzzle_json(
      puzzle_json
  )
  puzzle_data = puzzle_json.get("puzzle", {})
  ...
  puzzle_board_pil = generate_board_pil(
      board, arrows=[opponent_arrow] if opponent_arrow else None
  )
  ...
  for move_uci in solution_moves or []:
    try:
      ...
      if is_player_move:
        ...
        spoken_line = (
            "Now the opponent answers with"
            f" {san_move_to_spoken_text(san)}.{checkmate_suffix}"
        )
      ...
      solution_frames.append(
          create_reel_frame_array(
              generate_board_pil(
                  solution_board, arrows=[arrow], perspective=board.turn
              ),
              solution_hook,
              solution_side,
  ...
  if preview_mode:
    Image.fromarray(solution_frames[-1]).save(
        OUTPUT_DIR / "preview_frame.jpg", quality=92
    )

  # Audio Synthesis
  p_script, _ = convert_san_to_speech(side_name, first_solution_san)
  p_script = (
      f"The opponent's last move was {san_move_to_spoken_text(last_opponent_move)}."
      f" {p_script}"
  )
  ...
  background_file = OUTPUT_DIR / (
      "preview_background.wav" if preview_mode else "background.wav"
  )
  ...
  try:
    ...
    voice_intervals = [
        ...
        (
            puzzle_duration + 0.2,
            min(
                puzzle_duration + solution_duration,
                puzzle_duration + 0.2 + raw_v2.duration,
            ),
        ),
    ]
    ...
    for countdown in range(COUNTDOWN_SECONDS, -1, -1):
      segment_duration = min(
          puzzle_duration / (COUNTDOWN_SECONDS + 1), remaining_duration
      )
      ...
      puzzle_clips.append(
          mp.ImageClip(puzzle_frame).with_duration(segment_duration)
      )
    ...
    final_video = final_video.with_audio(
        mp.CompositeAudioClip([background, v1, v2, sfx])
    )

    print("🎬 Rendering Short MP4 video...")
